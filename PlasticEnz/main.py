...
def main():
    ...
    if args.test:
        ...
        print("🧪 Running PlasticEnz in test mode...")
        ...
        try:
            ...
            if shutil.which("signalp6"):
                ...
                print("🧪 SignalP enabled for test run (fast, auto).")
            else:
                print("ℹ️ signalp6 not found in PATH; skipping SignalP in test.")
        except Exception as _e:
            print(f"ℹ️ Could not check SignalP availability: {_e}")
    ...
    if args.genome:
        try:
            print(f"🧪Running Prodigal on genome/MAG file: {args.genome}")
            ...
            sys.exit(f"❌ERROR: Prodigal failed on genome/MAG file: {e}")
    elif args.contigs:
        try:
            print(f"🧪Running Prodigal on contigs: {args.contigs}")
            protein_out, gene_out, _ = run_prodigal(args.contigs, args.outdir, cores=args.cores, is_genome=False)
            protein_file = protein_out
        except Exception as e:
            logger.error(f"Error running Prodigal: {e}")
            sys.exit(f"❌ERROR: Prodigal failed on contigs file: {e}")
    else:
    ...
    for poly in valid_polymers:
        print(f"🧪Running HMMER for polymer: {poly}")
        try:
            outs = run_hmmer(
            ...
            sys.exit(f"❌ERROR: HMMER failed for {poly}: {e}")

        print(f"🧪Running DIAMOND for polymer: {poly}")
        try:
            outs = run_diamond(
                proteins=protein_file,
                polymer=poly,  # pass one polymer at a time
            ...
            sys.exit(f"❌ERROR: DIAMOND failed for {poly}: {e}")
    ...
    if args.reads_forward and args.reads_reverse:
        try:
            ...
            sys.exit(f"❌ERROR: Reads mapping failed: {e}")
            ...
            #except Exception as e:
                ...
                #sys.exit(f"❌ERROR: Failed to merge abundances: {e}")
    ...
    if prediction_polymers:
        ...
        print("ℹ️Skipping prediction step (PET and PHB not provided).")


    print("✅PlasticEnz analysis completed successfully!")
