...
def run_bowtie2(build_dir, reads_forward, reads_reverse, genes_file, outdir, sample_name, cores=1):
...
def calculate_abundance(bam_file, genes_file):
    ...
    for gene_id, data in gene_lengths.items():
        ...
        gene_lengths[gene_id]["CPM"] = (data["raw_count"] / total_mapped_reads) * 1e6
...
def process_multiple_samples(forward_reads, reverse_reads, genes_file, outdir, cores=1):
...
def run_mapping(forward_reads, reverse_reads, genes_file, outdir, cores=1):
    combined_df = process_multiple_samples(forward_reads, reverse_reads, genes_file, outdir, cores=cores)
